...
impl SignatureGenerator {
    pub fn make_signature_from_file(
        file_path: &str,
    ) -> Result<DecodedSignature, CriticalErrorKind> {
    ...
    fn do_fft(&mut self, s16_mono_16khz_buffer: &[i16]) {
        ...
        for (index, multiplier) in HANNING_WINDOW_2048_MULTIPLIERS.iter().enumerate() {
            self.reordered_ring_buffer_of_samples[index] = f32::from(
                self.ring_buffer_of_samples[(index + self.ring_buffer_of_samples_index) & 2047],
            ) * multiplier;
        }
                ...
                && fft_minus_46[bin_position] >= fft_minus_49[bin_position - 1]
            {
                ...
                if fft_minus_46[bin_position] > max_neighbor_in_fft_minus_49 {
                    ...
                    if fft_minus_46[bin_position] > max_neighbor_in_other_adjacent_ffts {
                        ...
                        let frequency_hz: f32 = f32::from(corrected_peak_frequency_bin)
                            * (16000.0 / 2.0 / 1024.0 / 64.0);
